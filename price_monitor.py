...
# from apscheduler.schedulers.background import BackgroundScheduler # Não precisamos importar a classe real aqui para type hinting
...
class BackgroundScheduler:
     ...
     def add_job(self, func, trigger, minutes, id): pass
     def start(self): pass
...
class PriceMonitor:
    ...
    def start(self):
        ...
        # run_alerts_check será executado a cada 1 minuto ('interval', minutes=1)
        # replace_existing=True evita duplicidade se start() for chamado mais de uma vez
        self.scheduler.add_job(self.run_alerts_check, 'interval', minutes=1, id='alerts_check_job', replace_existing=True)
        logger.info("Scheduled 'alerts_check_job' to run every 1 minute.")
